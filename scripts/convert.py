...
# --- FORMATTING HELPERS ---
STOP_WORDS = {'of', 'the', 'and', 'to', 'at', 'from', 'in', 'with', 'for', 'a', 'an'}
...
def to_title_case(text):
	...
	def capitalize_word(match):
		...
		# Check if it is the very first word of the entire string
		is_first_word = match.start() == 0

		# Check if it is the first word inside a parenthesis or after a hyphen
		preceding_char = text[match.start() - 1] if match.start() > 0 else ''
		is_after_paren = preceding_char == '('
		is_after_hyphen = preceding_char == '-'

		if is_first_word or is_after_paren or is_after_hyphen or word_lower not in STOP_WORDS:
			# Capitalize first letter, keep rest as is to protect apostrophes (e.g., Kree'arra)
			return word_lower.capitalize()
		else:
			return word_lower

	# Regex matches alphanumeric blocks, optionally containing a single internal apostrophe
	return re.sub(r"[a-zA-Z0-9]+(?:'[a-zA-Z0-9]+)?", capitalize_word, text)
...
def process_data(rates_reader, map_reader, kph_col, exp_col):
	...
	for row in rates_reader:
		# Format the activity name directly using the new helper
		name = to_title_case(row['Activity name'].strip())
	...
	for row in map_reader:
		# Format the activity name here as well to ensure perfect dictionary key matching
		activity_name = to_title_case(row['Activity name'].strip())
